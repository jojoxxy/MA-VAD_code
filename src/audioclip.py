...
class AudioCLIP(CLIP):
    ...
    def __init__(self,
        ...
        if isinstance(self.pretrained, str):
            self.load_state_dict(torch.load(self.pretrained,map_location='cuda:1'), strict=False)
            print('AudioCLIP weights loaded')
    ...
    def encode_token(self, tokens):
        return super(AudioCLIP, self).encode_token(tokens)

    def encode_text(self, text, token):
        return super(AudioCLIP, self).encode_text(text, token)
    '''
